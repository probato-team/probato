...
public class DatabaseNoSqlComponentValidator extends ComponentValidator {

	private static final String NOSQL_LIST_ANNOTATION_REQUIRED = "List of sql must have at least 1 item in the '@NoSQL' annotation: ''{0}''";
	private static final String NOSQL_VALUE_ANNOTATION_REQUIRED = "List of sql must not have null or empty value in the '@NoSQL' annotation: ''{0}''";
	...
	private void validateNoSQL(Class<?> clazz) {
		...
		NoSqlLoader.getNoSqlPaths(clazz).entrySet()
			.forEach(datasource -> {
				...
				validateNoSqlList(clazz, sqlList);
				sqlList.stream()
					.forEach(item -> {
	...
	private void validateSqlCommands(Class<?> clazz, String datasourceName, String item) {
		try {
			...
			var commands = NoSqlUtils.getDocuments(item);
			NoSqlUtils.validateDocuments(
					datasource.getUrl(),
